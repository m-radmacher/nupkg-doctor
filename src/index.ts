...
async function run() {
	...
	// Validate Version
	if (version) {
		const versionRegex = /^([0-9]{1,}).([0-9]{1,}).([0-9]{1,})(.([0-9]{1,}))?$/;
	...
	let nupkgFile: string | undefined;
	...
	let nuspecFile: string | undefined;
	...
	output.on('close', () => {
		console.log(
			`Wrote new .nupkg (${arch.pointer()} bytes). ${pushToReg ? 'Pushing .nupkg to GitHub registry...' : ''}`,
		);
	});
	arch.on('error', (err) => {
